...
Figure~\ref{fig:microcircuit_power} shows the power usage over time for simulations of the microcircuit model running for \SI{10}{\second} of biological time on each of the devices listed in table~\ref{tab:gpu_devices} aside from the Tesla V100 which we do not have local access to to repeat the methodology.
However both the Tesla cards have built in power monitoring which suggested that the Tesla V100 only drew a maximum of \SI{88}{\watt} compared to \SI{107}{\watt} for the Tesla K40m .
Assuming that a Tesla V100 was attached to the same workstation used for the Tesla K40m simulations we can therefore estimate that the total energy to solution would be \todo{XXX} kWh and the energy per synaptic event would be \todo{YYY} J.
