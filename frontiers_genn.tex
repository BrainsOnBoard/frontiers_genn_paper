...
\begin{figure}
    \begin{center}
        \includegraphics[width=18cm]{figures/potjans_circuit_v2}
    \end{center}
...
\begin{figure}
    \begin{center}
        \includegraphics[width=8cm]{figures/circuit2}
    \end{center}
...
The increase in the peak performance of the Tesla K40c (released in 2013) and the Tesla V100 (released in 2017) listed in table~\ref{tab:gpu_devices} illustrate how much GPUs have benefited from Moore's law scaling.
However, the modelling study performed by \citet{Esmaeilzadeh2012} indicates that the scaling of GPU performance will begin to slow as the `power wall' is reached.
Furthermore, GPU acceleration of SNN simulations is likely to be constrained by GPU memory bandwidth.
Although emerging technologies such as third generation High Bandwidth Memory~(HBM3) are likely to offer increases in memory bandwidth in the future, alternative strategies are still going to be required to better utilise current GPUs for SNN simulation as well as to increase the size of models that can be simulated using embedded GPUs such as the Jetson TX2.
This point is reflected in our simulations where, compared to the older Tesla K40c GPU, the cortical microcircuit model ran approximately twice as fast on the newer Tesla V100 GPU, potentially just reflecting its higher memory bandwidth, but the simulations of the plastic model ran more than $10\times$ faster on the V100.
As shown in figure~\ref{fig:stdp_performance}, the higher speedup is almost entirely due to the reduction in time spent in the postsynaptic learning kernel.
This kernel is where synaptic potentiation is applied using an additional column-major sparse matrix structure to select weights for updates in response to postsynaptic spikes, as outlined in section~\ref{sec:method/balanced_random}.
...
In our simulations of both models, we copy all spikes from the GPU to the host computer at the end of each simulation timestep -- accounting for the majority of the `overhead' shown in figures~\ref{fig:microcircuit_performance}~and~\ref{fig:stdp_performance}.
Because the microcircuit model has four times as many neuronal populations as the balanced random network model, copying its spiking output requires more interactions with the GPU driver, resulting in the higher overhead seen in simulations of this model.
...
However, because the Jetson TX2 is a system-on-chip in which the CPU and GPU cores share the same physical memory, no copying of data over the PCI Express bus is required.
...
Furthermore, based on the analysis we presented in section~\ref{sec:results/correctness}, this potential source of error did not appear toaffect our simulations suggesting that using single-precision floating point and summing inputs in a non-deterministic order has a minimal effect on the dynamics of the microcircuit model.
