...
Nonetheless, by employing the latest in memory and fabrication technologies, GPUs can still be very competitive for simulating SNNs in terms of speed and energy efficiency.
...
However, when simulating SNNs, GPU performance is likely to be bound by the memory bandwidth of the device.
Although new, even higher speed memory technologies such as HBM3~\todo{citation} will double the memory bandwidth available compared to HBM2 devices such as the Tesla V100, this comes at a significant power cost meaning that these memory technologies are unlikely to make their way into embedded systems.

Therefore alternative strategies are going to be required to best utilise future GPUs for SNN simulation as well as to increase the size of models that can be simulated using current embedded GPUs such as the Jetson TX2 in the type of neurorobotic applications discussed in section~\ref{sec:discussion/neurobotics}.
...
However these applications and the growing need to deploy ready-trained ANNs to perform inference in real time on low-power `edge computing' devices means that, here to, available memory bandwidth is beginning to limit performance.
...
Although training artidis a somewhat less As the ANNs used in machine learning continue to grow
...
JK is currently the primary GeNN developer.
JK performed the experiments whose results are presented in this work.
