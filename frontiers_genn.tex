...
For example the following `LIF' class describes a very simple leaky integrate-and-fire neuron with normalised units, solved algebraically:
%
...
%
The \lstinline{DECLARE_MODEL} and \lstinline{IMPLEMENT_MODEL} macros define boilerplate code used for subsequently defining parameters and initial model states.
\todo{sim code, parameters, values, derived parameters}
Macros are used to provide some `syntactic sugar' and type safety but are entirely optional.
GeNN uses this same approach for defining synapse models with the option to define code snippets for continuous time-driven updates as well as event-driven updates in response to pre and postsynaptic spikes.
Once r
%
...
%
This also demonstrates how the same approach used for defining models can be used to initialise variables.
Here the membrane voltage for our \num{1000} LIF neurons is sampled from the uniform distribution using a `variable initialisation snippet' built into GeNN which allows the same parallelisation strategies used for simulating models on the GPU to be applied to initialising them.
This approach is advantageous both as it removes the need to transfer the model state from the CPU to the GPU
...
Models can be implemented using the C++ interface described in the preceding section and the resultant simulation code can be linked against 
...
The type of one-shot, online learning observed in nature is one of the many properties that make spiking neural networks attractive when compared to the approaches typically employed in training artificial neural networks.
...
\begin{figure}[h!]
    ...
    Repeat as  necessary for each of your figures}\label{fig:microcircuit_accuracy}
\end{figure}
