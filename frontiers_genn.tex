...
Applying synaptic potentiation (equation~\ref{eq:stdp_update_post_pair}) is a little more tricky as it needs to be applied to each synapse in the column of the data structure shown in figure~\ref{fig:ragged_matrix} associated with a spiking postsynaptic neuron.

Some small-scale neuromorphic systems have tried to solve this problem in hardware using custom SRAM memories which allow both column-wise and row-wise accesses~\citep{Seo2011}.
However this is expensive in terms of silicon area so many neuromorphic systems avoid the problem entirely by implementing synaptic plasticity rules where the postsynaptic membrane voltage rather than the spike times is required meaning that no \textit{backpropagation} of postsynaptic spikes is required~\citep{Frenkel2018,Qiao2015}.
Intel's Loihi system~\citep{Davies2018} defers all STDP updates triggered by postsynaptic spikes until the end of a ``learning epoch'' after which time they are processed sequentially using 
NEST~\todo{cite} and SpiNNaker~\citep{Knight2016} both buffer postsynaptic spikes until the next presynaptic spike occurs meaning that they can be applied in order without having to make column-wise updates to the synaptic matrix. 
However this approach makes access to postsynaptic variables difficult as the previous state of postsynaptic variables would need to be buffered for an indeterminate length of time until the next presynaptic spike occurs.
