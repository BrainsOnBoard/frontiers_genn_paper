...
This point is reflected in our simulations where the cortical microcircuit model ran approximately twice as fast on the newer Tesla V100 GPU, potentially just reflecting its higher memory bandwidth, but the simulations of the plastic model ran more than $10\times$ faster on the V100 GPU.
...
However, because SpiNNaker is a software-programmable system, these limitations are not set in stone and \citet{Knight2016b} present some potential solutions to these issues.
\citeauthor{Knight2016b} showed both how models can be distributed more efficiently across a SpiNNaker machine and how Poisson background input could be directly injected into neurons in a similar manner to that presented in section~\ref{sec:method/microcircuit} -- reducing the cost of incoming spike processing.
Therefore it seems possible that, purely through software improvements, SpiNNaker could become much more energy efficient -- perhaps closer to the \SI{0.11}{\micro\joule} per synaptic event measured by \citet{Sharp2012} on a less complex model.

However, neuromorphic systems which use custom digital circuits to model individual neurons and synapse, require an order of magnitude less power with Intel's Loihi~\citep{Davies2018} system and IBM's TrueNorth~\citep{Merolla2014} both using in the order of \SI{10}{\pico\joule} per synaptic event.
Furthermore, subthreshold analogue systems such as the Dynapse system~\citep{Qiao2015} only use in the order of \SI{100}{\femto\joule} per synaptic event.

While the next generation SpiNNaker system will support floating point arithmetic and is likely to include bespoke accelerators to provide acceleration for some common transcendental functions~\citep{Mikaitis2018}.
