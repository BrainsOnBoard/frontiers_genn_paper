...
However, while neuromorphic systems may be the ultimate platform for \textit{deploying} spiking neural networks, their distributed nature and optimisation for specific types of models makes them unwieldy tools for \textit{developing} SNN models.
...
Over the last decade, as well as becoming a common fixture in many workstations, NVIDIA GPU accelerators have entered the High Performance Computing~(HPC) field and are now used in \SI{50}{\percent} of the Top 10 super computing sites to increase peak performance while lowering power requirements.
In many ways, the large amount of available parallelism and the Single Instruction Multiple Thread (SIMT) architecture of GPUs is well suited to simulating large numbers of homogeneous neurons and synapses.
However, this large amount of available parallelism comes at the expense of the SRAM which typically occupies a large proportion of the die area of neuromorphic chips, meaning that GPU performance is often constrained by external memory bandwidth.
Nonetheless, driven by the demands of the deep-learning community, GPUs employ the latest in memory and fabrication technologies making them very competitive for simulating SNNs in terms of speed and energy efficiency.

In this paper we use our GeNN code generator to re-implement two large-scale SNNs with levels of connectivity and sparseness matching those in the neocortex and discuss how they are mapped to GPU hardware.
We then verify the correctness of our GPU simulations against prior results obtained with NEST running on traditional HPC hardware and compare the performance with respect to speed and energy consumption against published data from CPU-based HPC and neuromorphic hardware.
...
\todo{not sure if this belongs at the end of the abstract}
Being able to rapidly initialise large SNN models enables rapid iteration and parameter sweeping -- both vital parts of an effective development workflow.
To support this, as well as generating code to simulate SNNs using GPU hardware, GeNN also parallelises their \textit{initialisation}.

\todo{this abstract needs to be < 350 words}
\tiny
...
Currently, the most common way to accelerate large-scale SNN simulations is to use CPU-based HPC clusters running software simulators such as NEST~\citep{Gewaltig2007} or parallel Neuron~\citep{carnevale2006neuron}.
...
Once network models have been defined using the C++ interface described in the preceding section, GeNN will generate the CUDA code to initialise and simulate the network which can then be linked against a simulation loop provided by the user:
...
This model of \SI{1}{\milli\metre\cubed} of early-sensory cortex was developed by \citet{Potjans2012} and consists of around \num{80000} neurons, divided into layers 2/3, 4, 5 and 6 -- each of which is modelled as an excitatory and an inhibitory population.
...
The first stage in initialising such connectivity between a presynaptic population with $N_{pre}$ neurons and a postsynaptic population with $N_{post}$ neurons is to determine how many of the total synapses $N$ end up in each row by sampling from the multinomial distribution $Mult[N_{pre} * N_{post}, \{P_{row}, P_{row}, \ldots, P_{row}\}]$ where $P_{row} = \frac{N_{post}}{N}$.
...
Again, for the full description of this model, please refer to \citeauthor{Morrison2007}.
