...
However, this large amount of available parallelism comes at the expense of the on-chip memory which typically occupies a large proportion of the die area of neuromorphic chips, meaning that GPU performance is often constrained by external memory bandwidth.
While single GPUs are never going to be capable of simulating the largest models currently simulated using CPU-based HPC, by employing the latest in memory and fabrication technologies, GPUs are very competitive for simulating circuit-scale ($10^5$--$10^6$ neurons) SNNs in terms of speed and energy efficiency.

In this paper we use our GeNN code generator to re-implement two circuit-scale point neuron network models with levels of connectivity and sparseness matching those in the neocortex and discuss how they are mapped to GPU hardware.
