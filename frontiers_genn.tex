...
However over the last decade, as well as being present in a large proportion of workstations, NVIDIA GPU accelerators are now used in \SI{50}{\percent} of the Top 10 super computing sites to increase peak performance.
...
The model consists of around \num{80000} leaky integrate-and-fire neurons, divided into layers 2/3, 4, 5 and 6 -- each layer modelled as one population of excitatory and another of inhibitory neurons.
...
In addition to this synaptic input, each neuron in the network also receives independant Poisson inputs with population-specific rates representing input from adjacent cortical regions.
Beside this basic structure, the connectivity is otherwise random with both synaptic strengths and transmission delays being normally distributed.
...
However the model that \citet{Morrison2007} develop in order to show how 
\citet{Morrison2007} argue that because STDP is, by nature, sensitive to correlations in neural activity experiments on the role of STDP in cortical dynamics made on scaled down models with fewer incoming synapses per neuron with larger synaptic weighhts.
\begin{align}
    \Delta w & = \
        \begin{cases}
...
CUDA abstracts away the amount of parallelism the application exposes from the amount of hardware parallelism available so we can run a model that uses \num{80000} threads on a GPU with \num{256} CUDA cores.
However memory is more of a constraint and, because of this, while all of the GPUs listed in table~\ref{tab:gpu_devices} can run the microcircuit model, only the 2 HPC GPUs have enough memory to run the STDP model.
