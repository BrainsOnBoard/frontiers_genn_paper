...
Besides performance in terms of the speed and energy consumption of the simulation, it has recently been identified that efficient initialisation of the model is a crucial concern, particularly in a research context where repeated runs and parameter-space exploration are required. 
Therefore, in this paper, we also introduce some of the novel parallel initialisation methods implemented in the latest version of GeNN and demonstrate how they can enable further speed and energy advantages.
...
\begin{figure}
    ...
    $i_{0},\ldots,i_{3}$ contain the indices of presynaptic spikes.
    Snaking lines indicate CUDA threads.
    Hatching indicates bits which are set.}
    \label{fig:bitmask}
\end{figure}
...
While this results in significant memory savings, due to the extra memory required by the column-major matrix used for STDP, this is still not enough to fit the model onto a GPUs with \SI{12}{\giga\byte} of memory.
...
However, computational constraints have meant that these systems have had to be operated either in simulation or their brain-inspired controllers have had to be simulated externally to the robot.
While using an external controller removes any constraints on its power and weight, it also introduces latency, meaning robots must operate slower as well as `tethering' the robot to a WiFi base station restricting where it can be operated.

The low power requirements and real-time performance of neuromorphic systems make them obvious candidate for building on-board neuro robotics controllers.
