...
\section{}
While neuromorphic systems may be the ultimate platform for \textbf{deploying} spiking neural networks~(SNNs), their distributed nature and optimisation for specific types of models makes them unwieldy tools for \textbf{developing} them.
...
Additionally, from a software engineering point-of-view, separating solving these equations allows for better encapsulation of neurons and synapses.
...
Scaling is performed by using the downscaling rules described by \citet{VanAlbada2015} to produce versions of the microcircuit model with a total of $N_t$ neurons using on a scaling factor $K = \frac{N_t}{77169}$.
...
Figure~\ref{fig:microcircuit_performance_scaling} shows the results of these simulations and suggests that, as at scales there are many more neurons than CUDA cores and activity remains constant, simulation times scale approximately linearly with the number of neurons and synapses.
