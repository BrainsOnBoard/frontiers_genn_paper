...
However, while the initialisation phase of the NEST simulation was fully parallelised and took less than \SI{3}{\second} in the configuration shown in figure~\ref{fig:microcircuit_performance}, the `FixedNumberTotal' connection rule used by the microcircuit model 
...
By integrating the power time series using the \lstinline{numpy.trapz} function we we can calculate the energy to solution of each simulation as well as the energy per synaptic event -- a common measure for comparing the energy efficiency of neuromorphic systems.
While we were unable to measure the energy of the Tesla V100 system directly, both the Tesla cards have built in power monitoring which showed that the Tesla V100 drew a maximum of \SI{88}{\watt} compared to \SI{107}{\watt} for the Tesla K40c.
