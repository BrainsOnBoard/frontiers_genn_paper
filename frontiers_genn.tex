...
As \citet{Rotter1999} describes, this approach leads to a delay of one simulation timestep compared to the exact solution, but this seperation is not only attractive from an engineering point-of-view by better encapsulating the neurons and synapses, but it also provides a generic way of describing neurons with heterogeneous synaptic input dynamics.
...
Simulating the sparsely connected synapses between two such populations of neurons is, at first glance, less suitable for GPU parallelism however, on modern GPU hardware, this can also be implemented in a very efficient manner.
...
\begin{align}
    \tau_{syn} \frac{dI_{s_{j}}}{dt} = & -I_{s_{j}} + \sum_{i=0}^{n} w_{ij} \sum_{t_{i}^{f}}  \delta(t - t_{i}^{f})\label{eq:alpha_neuron_input_current}
\end{align}
