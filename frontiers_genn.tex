...
One of these 2D arrays contains the indices of the postsynaptic neurons ($i$) and additional arrays are allocated for any individual synaptic state variables such as the synaptic weight~($w_{ij}$) or dendritic delay~($d_{ij}$).
...
\end{align}
%
In GeNN, if a neuron has fired in the current timestep, its trace variables are updated in the neuron kernel and synaptic depression (equation~\ref{eq:stdp_update_pre_pair}) is applied in the synapse kernel described in the previous section.
...
Intel's Loihi system~\citep{Davies2018} takes an alternative approach and defers all STDP updates until the end of a ``learning epoch'' after which time they are processed sequentially row by row.
NEST~\citep{Morrison2007} and SpiNNaker~\citep{Knight2016} both buffer postsynaptic spikes until the next presynaptic spike occurs -- allowing weight updates triggered by pre and postsynaptic spikes to be applied in order without having to make any column-wise accesses to the synaptic matrix. 
...
While, the communication latencies would, NVIDIA's GPUDirect~~\citep[Section~3.2.5.6.2]{NVIDIACorporation2018} technology allows data to be transferred directly between GPUs via compatible network cards rather than having to go via the host computers main memory.
