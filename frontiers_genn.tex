...
When a neuron's spiking threshold condition is met, the thread simulating the neuron writes its index within the population to a shared memory array.
After all threads in the thread blocks used to simulate a population have completed, their shared memory arrays are combined together into a global memory array containing the indices of all the neurons in the population which have spiked during the current simulation timestep.
