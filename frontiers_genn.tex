...
\begin{align}
    \tau_{syn} \frac{dI_{s_{j}}}{dt} = & x_{s_{j}} - I_{s_{j}} \\ 
    \tau_{syn} \frac{dx_{s_{j}}}{dt} = & -x_{s_{j}} + \sum_{i=0}^{n} w_{ij} \sum_{t_{i}^{f}}  \delta(t - t_{i}^{f})
\end{align}
...
However, in the model described in this section, \citet{Morrison2007} chose to use a rule with the following relationship between spike timing ($\Delta t = t_{post} - t_{pre}$)
\begin{align}
    \Delta w_{ij} & = \
        \begin{cases}
