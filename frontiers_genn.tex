...
\title[GPUs outperform current HPC and neuromorphic hardware]{GPUs outperform current HPC and neuromorphic hardware in terms
...
\end{align}
%
In GeNN, if a neuron has fired in the current timestep, its trace variables are updated in the neuron kernel \todo{do we need to say before what neuron kernel, synapse kernel etc are?} by evaluating equation~\ref{eq:stdp_trace}.
...
Similarly, applying synaptic potentiation involves applying equation~\ref{eq:stdp_update_post_pair} to each synapse targetting by a spiking postsynaptic neuron\todo{something wrong here - delete ``by''?}.
...
While using an external controller removes any constraints on its power and weight, it also introduces latency, meaning robots must operate slower, and `tethering' to a WiFi base station, meaning that they are restricted in where they operate.
...
The results of such simulations are then analysed offline to determine whether a particular combination of parameters has resulted in a successful\todo{correct word - TN: changed; any better?} emulation of a brain circuit.
